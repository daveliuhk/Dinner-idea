(function(){
  ...
  function categoryForIngredient(code){
    const i = currentData().ingredients.find(x=>x.code===code) || window.DEFAULT_DINNER_DATA?.ingredients?.find(x=>x.code===code);
  ...
  function categoryForBlock(code, row){
    const b=currentData().blocks.find(x=>x.code===code) || window.DEFAULT_DINNER_DATA?.blocks?.find(x=>x.code===code);
  ...
  function nutrition(recipe){
    ...
    Object.entries(recipe?.ingredients || {}).forEach(([code,q])=>{
      ...
      t.kcal+=Number(q)*Number(n.kcal||0); t.p+=Number(q)*Number(n.p||0); t.c+=Number(q)*Number(n.c||0); t.f+=Number(q)*Number(n.f||0);
    });
  ...
  function addNutrition(card, recipe, anchor){
    if(!card || !recipe || card.querySelector(":scope > .v13-nutrition, .recipe-mini > .v13-nutrition")) return;
    const n=nutritionNode(recipe);
    if(anchor?.parentNode) anchor.parentNode.insertBefore(n, anchor.nextSibling); else card.appendChild(n);
  }

  function enhanceWeek(){
    $$("#weekGrid .day-card").forEach(card=>{
      const sel=card.querySelector("select.day-select"); const r=recipeById(sel?.value); if(!r) return;
      addNutrition(card,r,card.querySelector(".recipe-mini .meta"));
    });
  }
  function enhanceMatches(){
    $$("#recipeMatches .match-card").forEach(card=>{
      const id=card.querySelector(".add-match")?.dataset.id; const r=recipeById(id); if(!r) return;
      addNutrition(card,r,card.querySelector(".match-details"));
    });
  }
  function enhanceRecipes(){
    ...
    if(!direct.length){
      $$(".recipe-card",root).forEach(card=>{ const id=card.querySelector(".recipe-edit")?.dataset.id; addNutrition(card,recipeById(id),card.querySelector(".meta")); });
      return;
    }
    const groups=new Map();
    direct.forEach(card=>{
      const id=card.querySelector(".recipe-edit")?.dataset.id; const r=recipeById(id);
      addNutrition(card,r,card.querySelector(".meta"));
      const cat=r?.category || card.querySelector(".chip")?.textContent?.trim() || "Other";
      if(!groups.has(cat)) groups.set(cat,[]); groups.get(cat).push(card);
    });
    const frag=document.createDocumentFragment();
    [...groups.keys()].sort().forEach(cat=>{
      const section=document.createElement("section"); section.className="category-section";
      const head=document.createElement("div"); head.className="group-heading-row"; head.innerHTML=`<h3 class="group-heading">${esc(cat)}</h3><span class="group-count">${groups.get(cat).length} recipes</span>`;
      const grid=document.createElement("div"); grid.className="recipe-grid"; groups.get(cat).sort((a,b)=>a.querySelector("h3").textContent.localeCompare(b.querySelector("h3").textContent)).forEach(x=>grid.appendChild(x));
      section.append(head,grid); frag.appendChild(section);
    });
  ...
  function enhanceEntities(rootSel, kind){
    ...
    rows.forEach(row=>{
      ...
      if(!groups.has(cat)) groups.set(cat,[]); groups.get(cat).push(row);
    });
    const frag=document.createDocumentFragment();
    [...groups.keys()].sort().forEach(cat=>{
      const section=document.createElement("section"); section.className="category-section";
      const head=document.createElement("div"); head.className="group-heading-row"; head.innerHTML=`<h3 class="group-heading">${esc(cat)}</h3><span class="group-count">${groups.get(cat).length} ${kind==="block"?"blocks":"items"}</span>`;
      const l=document.createElement("div"); l.className="entity-list"; groups.get(cat).forEach(x=>l.appendChild(x)); section.append(head,l); frag.appendChild(section);
    });
    root.replaceChildren(frag);
  }
  ...
  function regroupStock(){
    ...
    if(direct.length){
      const groups=new Map();
      direct.forEach(row=>{
        const code=row.querySelector(".stock-qty")?.dataset.code; const cat=categoryForIngredient(code);
        if(!groups.has(cat)) groups.set(cat,[]); groups.get(cat).push(row);
      });
      const frag=document.createDocumentFragment();
      [...groups.keys()].sort().forEach(cat=>{
        const sec=document.createElement("section"); sec.className="stock-category-group"; sec.dataset.category=cat;
        const h=document.createElement("h4"); h.className="group-heading compact"; h.textContent=cat; sec.appendChild(h);
    ...
    $$("#stockInputs .stock-category-group").forEach(sec=>{ sec.hidden=!!stockCategory && sec.dataset.category!==stockCategory; });
  }
  ...
  function renderSauces(){
    ...
    const q=($("#sauceSearch")?.value||"").toLowerCase(); const data=currentData();
    ...
    $$(".sauce-recipe-link",root).forEach(b=>b.addEventListener("click",()=>{
      ...
      const r=recipeById(b.dataset.id); const search=$("#recipeSearch"); if(search&&r){search.value=r.name;search.dispatchEvent(new Event("input",{bubbles:true}));}
    }));
  }

  function enhanceAll(){
    if(busy) return; busy=true;
    try { enhanceWeek(); enhanceMatches(); enhanceRecipes(); enhanceEntities("#blockList","block"); enhanceEntities("#ingredientList","ingredient"); resetStockCategorySelect(); wireStockFilter(); regroupStock(); } finally { busy=false; }
  }
  const observer=new MutationObserver(()=>queueMicrotask(enhanceAll));
  ["weekGrid","recipeMatches","recipeList","blockList","ingredientList","stockInputs","stockTypeFilter"].forEach(id=>{const el=document.getElementById(id); if(el) observer.observe(el,{childList:true,subtree:true});});
